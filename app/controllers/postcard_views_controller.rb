class PostcardViewsController < ApplicationController
  ...
  def list_tab
    ...
    if !params[:name].blank?
      list_tabs_with_name =Commodity.where("name like ? or short_name like ? or common_name like ?", "%#{params[:name]}%", "%#{params[:name]}%", "%#{params[:name]}%")

      @stamp_list_tabs =Commodity.joins_stamp.where("name like ? or short_name like ? or common_name like ? or stamps.serial_no like ?", "%#{params[:name]}%", "%#{params[:name]}%", "%#{params[:name]}%", "%#{params[:name]}%")

      @coin_list_tabs = list_tabs_with_name.joins_coin

      @bill_list_tabs = list_tabs_with_name.joins_bill
    else
      @stamp_list_tabs = @list_tabs.joins_stamp
      @coin_list_tabs = @list_tabs.joins_coin
      @bill_list_tabs = @list_tabs.joins_bill
    end
      ...
      if !params[:stamp_format].blank? or !params[:stamp_theme].blank? or !params[:coin_theme].blank? or !params[:coin_material].blank? or !params[:coin_weight].blank? or !params[:coin_year].blank? or !params[:coin_face_value].blank? or !params[:coin_pack_spec].blank? or !params[:bill_version].blank? or !params[:bill_engrave_year].blank? or !params[:bill_face_value].blank? or !params[:bill_pack_spec].blank? or !params[:bill_prefix].blank? or !params[:bill_serial_no].blank? or !params[:bill_watermark].blank?
        if !params[:stamp_format].blank? or !params[:stamp_theme].blank?
          if !params[:stamp_format].blank?
            ...
            @stamp_list_tabs = @stamp_list_tabs.with_stamp_format(params[:stamp_format]) if ! @stamp_list_tabs.blank?
          end

          if !params[:stamp_theme].blank?
            @checked_fileds += params[:stamp_theme]
            @stamp_list_tabs = @stamp_list_tabs.with_stamp_theme(params[:stamp_theme]) if ! @stamp_list_tabs.blank?
          end
        else
          @stamp_list_tabs = []
        end
        ...
        if !params[:coin_theme].blank? or !params[:coin_material].blank? or !params[:coin_weight].blank? or !params[:coin_year].blank? or !params[:coin_face_value].blank? or !params[:coin_pack_spec].blank?
          if !params[:coin_theme].blank?
            ...
            @coin_list_tabs = @coin_list_tabs.with_coin_theme(params[:coin_theme]) if ! @coin_list_tabs.blank?
          end

          if !params[:coin_material].blank?
            @checked_fileds += params[:coin_material]
            @coin_list_tabs = @coin_list_tabs.with_coin_material(params[:coin_material]) if ! @coin_list_tabs.blank?
          end

          if !params[:coin_weight].blank?
            @checked_fileds += params[:coin_weight]
            @coin_list_tabs = @coin_list_tabs.with_coin_weight(params[:coin_weight]) if ! @coin_list_tabs.blank?
          end

          if !params[:coin_year].blank?
            @checked_fileds += params[:coin_year]
            @coin_list_tabs = @coin_list_tabs.with_coin_year(params[:coin_year]) if ! @coin_list_tabs.blank?
          end

          if !params[:coin_face_value].blank?
            @checked_fileds += params[:coin_face_value]
             @coin_list_tabs = @coin_list_tabs.with_coin_face_value(params[:coin_face_value]) if ! @coin_list_tabs.blank?
          end

          if !params[:coin_pack_spec].blank?
            @checked_fileds += params[:coin_pack_spec]
            @coin_collections = @coin_list_tabs.with_coin_pack_spec(params[:coin_pack_spec]) if ! @coin_list_tabs.blank?
          end
        else
          @coin_list_tabs = []
        end
        ...
        if !params[:bill_version].blank? or !params[:bill_engrave_year].blank? or !params[:bill_face_value].blank? or !params[:bill_pack_spec].blank? or !params[:bill_prefix].blank? or !params[:bill_serial_no].blank? or !params[:bill_watermark].blank?
          if !params[:bill_version].blank?
            ...
            @bill_list_tabs = @bill_list_tabs.with_bill_version(params[:bill_version]) if ! @bill_list_tabs.blank?
          end

          if !params[:bill_engrave_year].blank?
            @checked_fileds += params[:bill_engrave_year]
            @bill_list_tabs = @bill_list_tabs.with_bill_engrave_year(params[:bill_engrave_year]) if ! @bill_list_tabs.blank?
          end

          if !params[:bill_face_value].blank?
            @checked_fileds += params[:bill_face_value]
            @bill_list_tabs = @bill_list_tabs.with_bill_face_value(params[:bill_face_value]) if ! @bill_list_tabs.blank?
          end

          if !params[:bill_pack_spec].blank?
            @checked_fileds += params[:bill_pack_spec]
            @bill_list_tabs = @bill_list_tabs.with_bill_pack_spec(params[:bill_pack_spec]) if ! @bill_list_tabs.blank?
          end

          if !params[:bill_prefix].blank?
            @checked_fileds += params[:bill_prefix]
            @bill_list_tabs = @bill_list_tabs.with_bill_prefix(params[:bill_prefix]) if ! @bill_list_tabs.blank?
          end

          if !params[:bill_serial_no].blank?
            @checked_fileds += params[:bill_serial_no]
            @bill_list_tabs = @bill_list_tabs.with_bill_serial_no(params[:bill_serial_no]) if ! @bill_list_tabs.blank?
          end

          if !params[:bill_watermark].blank?
            @checked_fileds += params[:bill_watermark]
            @bill_list_tabs = @bill_list_tabs.with_bill_watermark(params[:bill_watermark]) if ! @bill_list_tabs.blank?
          end
        else
          @bill_list_tabs = []
        end
    ...
    if !@stamp_list_tabs.blank? or !@coin_list_tabs.blank? or !@bill_list_tabs.blank?
      @commodities = @stamp_list_tabs + @coin_list_tabs + @bill_list_tabs      
    else
      @commodities = []
    end
